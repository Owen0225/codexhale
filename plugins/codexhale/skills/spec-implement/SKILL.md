...
**Next:** Verify phase (type-aware: `codexhale:spec-verify` for features, `codexhale:spec-bugfix-verify` for bugfixes)
...
`codexhale:spec-plan` should have called `ExitPlanMode` before invoking this skill. If it didn't (model skip, approval edge case, etc.), you are still on Opus in plan mode and implementation will run on the wrong model. Exit now.
...
**Then hand off to `codexhale:spec-bugfix-verify` (via Step 3 of this skill). Verify runs the suite once more as the authoritative final signal, plus a Behavior Contract audit and an always-on revert-test — quality insurance, not waste.**
...
5. Read `Type:` field.
   <!-- CC-ONLY -->
   Bugfix: `Skill(skill='codexhale:spec-bugfix-verify', args='<plan-path>')` | Otherwise: `Skill(skill='codexhale:spec-verify', args='<plan-path>')`
   <!-- /CC-ONLY -->
   <!-- CODEX-START
